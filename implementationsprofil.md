...
`I den mån vår implementationsprofil skiljer sig från Sambruks så är det markerat med grå bakgrund och utropstecken. Vi har listat vissa attribut som _önskvärda_ istället för _valfria_. Det betyder att vi gärna tar emot dem men de är inget krav från vår sida. Den enda avgörande skillnaden från Sambruk är att i fallet med emails på Users så är det _valfritt_ i Sambruks implementationsprofil men _krävs_ i vår profil. Det gör att huvudmannen är fri att sätta attributeet userName godtyckligt utan koppling till användarens userName på Kunskapsmatrisen och Exam.net (som förövrigt är epostadress).`
